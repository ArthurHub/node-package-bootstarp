...
async function bootstrap() {
  console.info(`Node.js Single Executable Bootstrap Application for "${metadata.name}"`);

  // create temp directory to unzip the bundle
  const tmpFolder = path.join(os.tmpdir(), BASE_FOLDER, metadata.name);
  ...
  if (!fs.existsSync(nodeExec)) {
    console.info(`extracting node executable`);
  ...
  // extract app sources
  console.debug(`Extracting app sources`);
  ...
  console.info(`Running ${node} ${bundle}`);
  console.clear();
