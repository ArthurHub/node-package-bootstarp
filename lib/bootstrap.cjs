 ...
 */
module.exports = async function bootstrap(argv = []) {
  try {
    ...
    debugFlag = metadata.debug || argv.indexOf('--debug') > -1;

    const sebPathArgvIdx = argv.indexOf('--seb-path');
    const baseFolder = sebPathArgvIdx > -1 ? argv[sebPathArgvIdx + 1] : path.join(os.tmpdir(), BASE_FOLDER);
